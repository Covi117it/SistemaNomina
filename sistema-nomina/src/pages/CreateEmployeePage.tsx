...
export const CreateEmployeePage: React.FC<CreateEmployeePageProps> = ({
  ...
  const { canEditEmployee } = useAuthorization();
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {/* Form Sections */}
         <form onSubmit={handleSubmit} className="space-y-6">
          <fieldset
            disabled={isEditMode && !canEditEmployee}
            className={
              isEditMode && !canEditEmployee
                ? 'space-y-6 pointer-events-none opacity-85 select-none border-none p-0 m-0'
                : 'space-y-6 border-none p-0 m-0'
            }
          >
          ...
          {(!isEditMode || canEditEmployee) ? (
            ...
            <div className="text-center pt-4">
              <span className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 text-amber-800 text-xs font-bold rounded-xl">
                Modo Consulta (Solo Lectura): No tienes permisos para modificar este empleado.
              </span>
            </div>
          )}
        </form>
